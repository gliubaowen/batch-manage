...
# -------------------------------------------------------------------------------
# Filename:    batch-download.sh 
# Version:     3.0.0
# Date:        2018-11-22
# Author:      LiuBaoWen
# Email:       bwliush@cn.ibm.com
# Description: 从远程批量下载文件
# Notes:       
# -------------------------------------------------------------------------------
...
if [ $# -lt 3 ]
    then
    echo "USAGE:$0 USAGE|PROPERTIES|FROM-HOST-FILE|TO-LOCAL-DIR"
    exit 1
fi
...
main()
{
	...
	do
		...
		echo "[info] @@@@@@@@@@@@@@@@@@@@start@@@@@@@@@@@@@@@@@@@@"
		bash ssh-download.sh $remote_host "${from_path}" ${to_path}
		echo "[info] @@@@@@@@@@@@@@@@@@@@end@@@@@@@@@@@@@@@@@@@@"
	done
